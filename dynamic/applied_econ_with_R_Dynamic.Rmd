...
t.test(test_score~letter,filter(analysisdata,year==4))
```

But what is the right interpretation of the test result?

```{r quiz2}
quiz(
  question("What is the correct interpretation of the test?",
    answer("The p-value is 0.65949, and using a 5% (or even higher) significance level we therefore cannot reject the null hypothesis that the difference in means between letter receivers and non-receivers is zero.In other words, the test score before was significantly different across letter receivers and not receivers." ),
        answer("The p-value is 0.5097, and using a 5% (or even higher) significance level we therefore cannot reject the null hypothesis that the difference in means between letter receivers and non-receivers is zero. In other words, the test score before was not significantly different across letter receivers and not receivers.", correct = TRUE),
       answer("The p-value is 0.5097, and using a 5% (or even higher) significance level we therefore  reject the null hypothesis that the difference in means between letter receivers and non-receivers is zero. In other words, the test score before was significantly different across letter receivers and not receivers.")
  )
)
```
