...
- There are two tutorials. This one is the **dynamic** version. To try the **static** version go to: [hhsievertsen.github.io/applied_econ_with_r/](https://hhsievertsen.github.io/applied_econ_with_r/) (they cover the same material).
...
Now it is your turn, in the block below we want to 
...
Now that we have merged the datasets,  we want to make sure that the merged dataset satisfies the  [Tidy Data Principles](https://vita.had.co.nz/papers/tidy-data.pdf). From these principles we know that the number of columns should correspond to the number of variables in the dataset. But how many variables are there?

```{r quiz}
quiz(
  question("How many variables does the merged dataset contain",
    answer("7"),
...
```{r ch3-1e,exercise=TRUE}
# Use the dim() functions to get the number of rows and columns
```
```{r ch3-1e-hint-1}
# Use the dim() functions to get the number of rows and columns
dim(school_data_merged)
...
Let's  change the appearance of the table and export it to Word.  When exporting it to Latex, we can't have histogram', so we disable them. We specify `output="docx"` to write to a Word file. We can also export the table to other formats like LaTex (note that this will not work in this dynamic version of the tutorial, but only on your own computer or in RStudio cloud). 
...
```{r ch4-2b,eval=T, warning=F,include=T,echo=F,message=FALSE}
# load modelsummary
library("modelsummary")
# create a summary stat table in Latex format
analysisdata%>%
...
This chart does not look very good. You can use the tools from the [R introduction](https://hhsievertsen.shinyapps.io/r_introduction_UoB_2/) to polish it (or you can use the hints to get some suggestions).
...
```{r merge4-hint-1}
# Make the line green by setting the color to the html colour code "#145c21"
...
```{r merge4-hint-2}
# Make the scatters green, partly transparent (with alpha=), and change the size of them.
...
```{r merge4-hint-3}
# Change the colour of the background and the grid lines
...
```{r merge4-hint-4}
# Change the axes titles and add an overall title
...
```{r merge5, exercise=TRUE,  warning=FALSE, message=FALSE}
...
```{r merge6, exercise=TRUE,  warning=FALSE, message=FALSE}
# Filter year 5
...
Note that  `df<-analysisdata%>%filter(year==5)` might look a bit unfamiliar to you. It simply a different way of writing `df<-filter(analysisdata,year==5)`. But what is the right interpretation of the test result?

```{r quiz2}
quiz(
  question("What is the correct interpretation of the test?",
    answer("The p-value is < 2.2e-16, and using a 5% significance level we therefore  reject the null hypothesis that the difference in means between those who attended and those who did not attend is zero. In other words, the test score before was significantly different across those who attended and those who did not attend." , correct = TRUE),
        answer("The p-value is < 2.2e-16, and using a 5% (or even higher) significance level we therefore cannot reject the null hypothesis that the difference in means between those who attended and those who did not attend is zero. In other words, the test score before was not significantly different across those who attended and those who did not attend."),
       answer("The p-value is 0.2082893 , and using a 5% (or even higher) significance level we therefore  reject the null hypothesis that the difference in means between those who attended and those who did not attend is zero. In other words, the test score before was significantly different across those who attended and those who did not attend.")
  )
)
```
...
Below is an example based on a univariate regression between testscore and the indicator for receiving the letter. Try to change the code to also include parental_lincome in the regression (see hints).

```{r merge7, exercise=TRUE,  warning=FALSE, message=FALSE}
# Our first ordinary least squares regression
lm(test_score~letter,data=filter(analysisdata,year==6))
```

```{r merge7-hint-1}
# ... now also with parental_lincome
lm(test_score~letter+parental_lincome,data=filter(analysisdata,year==6))
```
...
```{r merge8, exercise=TRUE,  warning=FALSE, message=FALSE}
# Summary of ordinary least squares regression
summary(lm(test_score~letter,data=filter(analysisdata,year==6)))
```

```{r merge8-hint-1}
# ... now also with parental lincome
summary(lm(test_score~letter+parental_lincome,data=filter(analysisdata,year==6)))
```


A nice feature of R is that we can assign basically everything to a name and then use it later. In the code block below we first run a regression and store it under the name *model1*. We then later call these regression results back when using the `summary()` command (see hints!). In the following example we include a few covariates in the regression: 


```{r merge9, exercise=TRUE,  warning=FALSE, message=FALSE}
...
```{r merge10, exercise=TRUE,  warning=FALSE, message=FALSE,exercise.setup = "regdata"}
# Load  modelsummary
library(modelsummary)
...
```{r ch8-1,exercise=T,exercise.setup = "regdata"}
...
We can use `glm()` to estimate logit and probit models. We specify the link function as an option as shown below for probit. 


```{r merge11, exercise=TRUE,  warning=FALSE, message=FALSE,exercise.setup = "regdata"}
...
```{r merge11-hint-1}
# ... now a the logit: we simply change the link to "logit"
...
Try to change the code above to get the logit estimates (see hints!). Let's compute the average marginal effects.
...
We estimated the reduced form relationship between receiving a reminder letter and test scores to be 0.2SD. We have also estimated the first stage to be around 0.4. What does this mean for the IV estimator (hint: use the Wald Estimator)? 


```{r quiz3}
quiz(
  question("Given the reduced form estimate of 0.2SD and a first stage coefficient of 0.4, the IV estimate is expected to be",
    answer("About 0.5  based on dividing the reduced form estimate of 0.2 by the first stage of 0.4.", correct = TRUE),
    answer("About 0.08  based on multiplying the reduced form estimate of 0.2 by the first stage of 0.4."),
    answer("About 0.2, because it is always the same as the reduced form")
  )
)
```


Let us estimate the instrumental variable specification explicitly to see whether it maps with our answer above. I will show you two methods. 
...
Note that in the code block below I included a few "REPLACEME". The code will not work unless you change them to the right code. 


```{r merge12, exercise=TRUE,  warning=FALSE, message=FALSE,exercise.setup = "regdata"}
# Estimate OLS
...
We use this as a change to first collapse the data on the year times summerschool level using `group_by()` and `summarise()`. There are a few "REPLACEME"s that you need to replace in order to make the code run. Hint: check the hints if you are stuck!


```{r merge14, exercise=TRUE,  warning=FALSE, message=FALSE,exercise.setup = "regdata"}
...
Note that I again added a few "REPLACEME" that you need to change before the code will run. Don't worry, you can always check the hints.


```{r merge13, exercise=TRUE,  warning=FALSE, message=FALSE,exercise.setup = "regdata"}
# Modify data
